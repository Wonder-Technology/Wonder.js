...
module MergePipelineData = {
  ...
  ): WonderCommonlib.Result.t2<IWorkForJs.pluginName> => {
  ...
  let _findAllSpecificPipelineRelatedData = (
    allRegisteredWorkPluginData: WorkManagerType.allRegisteredWorkPluginData,
    targetPipelineName: PipelineType.pipelineName,
  ...
  let _buildFirstGroupElement = (
    groups: PipelineType.groups,
    first_group,
  ): WonderCommonlib.Result.t2<PipelineType.element> => {
    groups
    ->WonderCommonlib.ArraySt.find(({name}) => {
      name === first_group
    })
    ->WonderCommonlib.OptionSt.map(({name}): PipelineType.element => {
      name: name,
      type_: #group,
    })
    ->WonderCommonlib.OptionSt.get
  ...
  let _insertElement = (
    ...
    insertAction,
    insertElementName,
    insertElement: PipelineType.element,
  ) => {
  ...
  let _mergeGroups = (
    groups: PipelineType.groups,
    insertGroups: PipelineType.groups,
  ): PipelineType.groups => {
  ...
  let _mergeToRootNode = (tree: TreeType.tree): WonderCommonlib.Result.t2<(
    list<IWorkForJs.getExecFunc<RegisterWorkPluginType.states>>,
    PipelineType.pipelineData,
  )> => {
