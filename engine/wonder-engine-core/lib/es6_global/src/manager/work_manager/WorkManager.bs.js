...
import * as OperateTree$WonderEngineCore from "./OperateTree.bs.js";
...
function _getStates(param) {
  return POContainer$WonderEngineCore.unsafeGetPO(undefined).states;
}

function _setStates(states) {
  var init = POContainer$WonderEngineCore.unsafeGetPO(undefined);
...
function _buildJobStream(execFunc) {
  ...
  return Most.map(_setStates, Most.flatMap((function (func) {
                    return Curry._1(func, POContainer$WonderEngineCore.unsafeGetPO(undefined).states);
                  }), __x));
}
...
function _handleInsertedAsRootNode(treeDataList, param) {
  ...
  return ListSt$WonderCommonlib.reduce(treeDataList, [
              ...
              undefined
                ...
                var insertedTree = TreeNode$WonderEngineCore.buildNode(pluginName, [
                      getExecFunc,
                      pipelineData,
                      nodeJobOrderOpt
                    ], treeData[0]);
              ...
              false
                ...
                var match = ListSt$WonderCommonlib.reduce(sameLevelTreeList, [
                      ...
                      false
                        ...
                        var match = OperateTree$WonderEngineCore.insertNode(tree, nodeInsertPluginName, node);
                        return [
  ...
  return ListSt$WonderCommonlib.filter(ListSt$WonderCommonlib.map(treeDataList, (function (param) {
                    return [
                            ListSt$WonderCommonlib.filter(param[0], (function (sameLevelTree) {
                                    return !TreeNode$WonderEngineCore.isEqual(sameLevelTree, insertedTree);
                                  })),
                            param[1]
                          ];
                  })), (function (param) {
...
function _buildTree(allSpecificPipelineRelatedData) {
  return _getTree(ListSt$WonderCommonlib.reduce(allSpecificPipelineRelatedData, /* [] */0, (function (treeDataList, param) {
                    ...
                    if (jobOrder !== undefined) {
                      ...
                      var node = TreeNode$WonderEngineCore.buildNode(pluginName, [
                            getExecFunc,
                            pipelineData,
                            nodeJobOrderOpt
                          ], /* [] */0);
                    ...
                    if (OptionSt$WonderCommonlib.isSome(match$3[1])) {
                      ...
                      return _add(treeDataList$4, TreeNode$WonderEngineCore.buildNode(pluginName, [
                                      getExecFunc,
                                      pipelineData,
                                      undefined
                                    ], /* [] */0), undefined);
...
function _mergeToRootNode(tree) {
  return Result$WonderCommonlib.mapSuccess(IterateTree$WonderEngineCore.postOrderCataWithParentNode((function (parentNodeOpt, pluginName, nodeData) {
                    var getExecFuncs = nodeData.getExecFuncs;
                    var pipelineData = nodeData.pipelineData;
                    var jobOrder = nodeData.jobOrder;
                    return function (children) {
                      var node = TreeNode$WonderEngineCore.buildNodeByNodeData(pluginName, nodeData, children);
                      if (parentNodeOpt === undefined) {
                        return Result$WonderCommonlib.succeed(node);
                      }
                      var parentNodeData = TreeNode$WonderEngineCore.getNodeData(parentNodeOpt);
                ...
                var match = TreeNode$WonderEngineCore.getNodeData(tree);
