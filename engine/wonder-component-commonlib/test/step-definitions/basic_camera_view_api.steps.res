...
defineFeature(feature, test => {
  ...
  test(."getViewWorldToCameraMatrix", ({given, \"when", \"and", then}) => {
    let gameObject = 1
    let basicCameraViewData: ref<WonderEngineCore.RegisterComponentType.usedComponentData> = ref(
      Obj.magic(1),
    )
    let transformData: ref<WonderEngineCore.RegisterComponentType.usedComponentData> = ref(
      Obj.magic(1),
    )
    ...
    \"and"({j`register transform data`}, () => {
      WonderEngineCore.Main.registerComponent(WonderComponentTransform.Main.getData()->Obj.magic)
    })
    ...
    \"and"("create and set all component states", () => {
      ...
      transformData := WonderEngineCore.Main.unsafeGetRelatedComponentData(transformComponentName)
      basicCameraViewData :=
        WonderEngineCore.Main.unsafeGetRelatedComponentData(basicCameraViewComponentName)
    })
    ...
    \"and"("create a transform", () => {
      let (d, t) = WonderEngineCore.Main.createComponent(transformData.contents)
    ...
    \"and"("add the transform to the gameObject", () => {
      transformData :=
        WonderEngineCore.Main.addComponent(
    ...
    \"and"("create a basicCameraView", () => {
      let (d, b) = WonderEngineCore.Main.createComponent(basicCameraViewData.contents)
  ...
  let _prepareDataForGetActiveCameraView = (
    ...
    let data: ref<WonderEngineCore.RegisterComponentType.usedComponentData> = ref(Obj.magic(1))
    let gameObject1 = 1
    let gameObject2 = 2
    ...
    \"and"("create two basicCameraViews as cameraView1, cameraView2", () => {
      data := WonderEngineCore.Main.unsafeGetRelatedComponentData(basicCameraViewComponentName)

      let (d, b1) = WonderEngineCore.Main.createComponent(data.contents)
      let (d, b2) = WonderEngineCore.Main.createComponent(d)
