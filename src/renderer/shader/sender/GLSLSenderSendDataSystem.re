...
let sendBuffer =  (gl, size: option(int), pos: option(attributeLocation), buffer: buffer, state: StateDataType.state) => {
  let pos = Js.Option.getExn(pos);
  let size = Js.Option.getExn(size);
