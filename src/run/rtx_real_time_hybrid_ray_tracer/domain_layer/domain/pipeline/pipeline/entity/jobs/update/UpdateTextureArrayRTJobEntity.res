...
let _setMapBetweenImageIdToLayerIndex = allUsedImageIdAndData =>
  allUsedImageIdAndData->ListSt.reduce(0, (layerIndex, (imageId, _)) => {
    TextureArrayWebGPURTRepo.setLayerIndex(imageId->ImageIdVO.value, layerIndex)
    ...
    : {
        ...
        while yy.contents < height {
          ...
          while xx.contents < width {
            ...
            TypeArrayRTRepoUtils.setUint8_1WithoutCheck(
              bufferDataIndex + 0,
              TypeArrayRTRepoUtils.getUint8_1(dataIndex + 0, data),
              bufferData,
            )
            TypeArrayRTRepoUtils.setUint8_1WithoutCheck(
              bufferDataIndex + 1,
              TypeArrayRTRepoUtils.getUint8_1(dataIndex + 1, data),
              bufferData,
            )
            TypeArrayRTRepoUtils.setUint8_1WithoutCheck(
              bufferDataIndex + 2,
              TypeArrayRTRepoUtils.getUint8_1(dataIndex + 2, data),
              bufferData,
            )
            TypeArrayRTRepoUtils.setUint8_1WithoutCheck(
              bufferDataIndex + 3,
              TypeArrayRTRepoUtils.getUint8_1(dataIndex + 3, data),
              bufferData,
            )
...
let _setWebGPUObjects = (textureArrayView, textureSampler) => {
  TextureArrayWebGPURTRepo.setTextureArrayView(textureArrayView)
  TextureArrayWebGPURTRepo.setTextureSampler(textureSampler)
}
...
let exec = () =>
  Tuple2.collectOption(WebGPURTRepo.getDevice(), WebGPURTRepo.getQueue())->Result.bind(((
    ...
    let (textureArrayLayerWidth, textureArrayLayerHeight) = WebGPURTAPI.getTextureArrayLayerSize()
