...
let getSceneDescBufferData = () =>
  CPRepo.getRayTracingPass().sceneDescBufferData->OptionSt.map(((buffer, bufferSize, typeArr)) => (
...
let getPointIndexBufferData = () =>
  CPRepo.getRayTracingPass().pointIndexBufferData->OptionSt.map(((buffer, bufferSize, typeArr)) => (
    buffer->StorageBufferVO.create,
    bufferSize,
    typeArr,
  ))
...
let getVertexBufferData = () =>
  CPRepo.getRayTracingPass().vertexBufferData->OptionSt.map(((buffer, bufferSize, typeArr)) => (
    buffer->StorageBufferVO.create,
    bufferSize,
    typeArr,
  ))
...
let getIndexBufferData = () =>
  CPRepo.getRayTracingPass().indexBufferData->OptionSt.map(((buffer, bufferSize)) => (
    buffer->StorageBufferVO.create,
    bufferSize,
  ))
...
let getBSDFMaterialBufferData = () =>
  CPRepo.getRayTracingPass().bsdfMaterialBufferData->OptionSt.map(((
    buffer,
    bufferSize,
    typeArr,
...
let getShaderBindingTable = () => CPRepo.getRayTracingPass().shaderBindingTable
...
let getAllStaticBindGroupData = () => CPRepo.getRayTracingPass().staticBindGroupDataList

let getPipeline = () => CPRepo.getRayTracingPass().pipeline
...
let getCameraBindGroupLayout = () => CPRepo.getRayTracingPass().cameraBindGroupLayout
