let getSampleCount = () => {
  CPRepo.getPass().sampleCount;
};

let setSampleCount = sampleCount => {
  CPRepo.setPass({...CPRepo.getPass(), sampleCount});
};

let getTotalSampleCount = () => {
  CPRepo.getPass().totalSampleCount;
};

let setTotalSampleCount = totalSampleCount => {
  CPRepo.setPass({...CPRepo.getPass(), totalSampleCount});
};

let getPixelBufferData = () => {
  CPRepo.getPass().pixelBufferData
...
let getCommonBufferData = () => {
  CPRepo.getPass().commonBufferData
  ->OptionSt.map(((buffer, typeArr)) => {
      (buffer->UniformBufferVO.create, typeArr)
    });
};
...
let getResolutionBufferData = () => {
  CPRepo.getPass().resolutionBufferData
  ->OptionSt.map(((buffer, typeArr)) => {
      (buffer->UniformBufferVO.create, typeArr)
    });
};
