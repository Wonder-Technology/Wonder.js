let getAccumulationPixelBufferData = () => {
  CPRepo.getAccumulationPass().accumulationPixelBufferData
...
let setAccumulationPixelBufferData = ((buffer, bufferSize)) => {
  CPRepo.setAccumulationPass({
    ...
    accumulationPixelBufferData:
      (buffer->StorageBufferVO.value, bufferSize)->Some,
  });
};

let getStaticBindGroupData = () => {
  CPRepo.getAccumulationPass().staticBindGroupData;
};

let setStaticBindGroupData = (setSlot, bindGroup) => {
  CPRepo.setAccumulationPass({
    ...CPRepo.getAccumulationPass(),
    staticBindGroupData: {setSlot, bindGroup}->Some,
  });
};

let getPipeline = () => {
  CPRepo.getAccumulationPass().pipeline;
};
