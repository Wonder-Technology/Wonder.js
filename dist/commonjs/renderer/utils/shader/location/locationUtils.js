...
exports.setLocationMap = contract_1.ensureFunc(function (returnVal, gl, shaderIndex, program, materialShaderLibConfig, shaderLibData, LocationDataFromSystem) {
    contract_1.it("attribute should contain position at least", function () {
        wonder_expect_js_1.expect(LocationDataFromSystem.attributeLocationMap[shaderIndex]["a_position"]).be.a("number");
    });
}, contract_1.requireCheckFunc(function (gl, shaderIndex, program, materialShaderLibConfig, shaderLibData, LocationDataFromSystem) {
    contract_1.it("not setted location before", function () {
        wonder_expect_js_1.expect(objectUtils_1.isValidMapValue(LocationDataFromSystem.attributeLocationMap[shaderIndex])).false;
        wonder_expect_js_1.expect(objectUtils_1.isValidMapValue(LocationDataFromSystem.uniformLocationMap[shaderIndex])).false;
    });
