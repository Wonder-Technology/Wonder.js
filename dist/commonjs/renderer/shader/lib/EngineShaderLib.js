...
var VariableLib_1 = require("../variable/VariableLib");
...
var EngineShaderLib = (function (_super) {
    ...
    function EngineShaderLib() {
        ...
        _this.attributes = Hash_1.Hash.create();
        _this.uniforms = Hash_1.Hash.create();
        _this.vsSourceTop = "";
    ...
    EngineShaderLib.prototype.sendAttributeBuffer = function (program, name, data) {
        program.sendAttributeBuffer(name, EVariableType_1.EVariableType.BUFFER, data);
    };
    EngineShaderLib.prototype.sendUniformData = function (program, name, data) {
        program.sendUniformData(name, VariableLib_1.VariableLib[name].type, data);
    };
    ...
    EngineShaderLib.prototype.setVsSource = function (vs, operator) {
        ...
        if (JudgeUtils_1.JudgeUtils.isString(vs)) {
            this.vsSource = vs;
        }
        else {
    ...
    EngineShaderLib.prototype.setFsSource = function (fs, operator) {
        if (operator === void 0) { operator = "="; }
        if (JudgeUtils_1.JudgeUtils.isString(fs)) {
            this.fsSource = fs;
        }
        else {
    ...
    EngineShaderLib.prototype._addVariable = function (target, variableArr) {
        variableArr.forEach(function (variable) {
            contract_1.assert(VariableLib_1.VariableLib[variable], Log_1.Log.info.FUNC_SHOULD(variable, "exist in VariableLib"));
            target.addChild(variable, VariableLib_1.VariableLib[variable]);
        });
    };
    ...
    EngineShaderLib.prototype._setSource = function (chunk, sourceType, operator) {
        ...
        switch (operator) {
            ...
            case "=":
                ...
                break;
            default:
                Log_1.Log.error(true, Log_1.Log.info.FUNC_INVALID("opertor:", operator));
                break;
        }
    };
...
__decorate([
    contract_1.requireCheck(function (program, name, data) {
        contract_1.assert(!!VariableLib_1.VariableLib[name], name + " should exist in VariableLib");
    })
...
__decorate([
    contract_1.requireCheck(function (target, variableArr) {
        variableArr.forEach(function (variable) {
            contract_1.it("should exist in VariableLib", function () {
                wonder_expect_js_1.default(VariableLib_1.VariableLib[variable]).exist;
            });
        });
    })
