...
function _runTest(browserArr, done) {
    ...
    testPerformance.runTest(browserArr).then(function () {
        console.log("done");
        done()
    }, function (e) {
        console.log("fail");
...
// todo refactor with testRender.js
gulp.task("testPerformance", function (done) {
    ...
    git.revParse({ args: "HEAD" }, function (err, commitId) {
        ...
        var basedCommitId = config.performance.base_commit_id;
        ...
        if (basedCommitId === config.performance.last_generate_based_commit_id) {
            _runBuild(function () {
                _runTest([], done);
            });
            return
