        ...
        () => {
          let _prepareForBasicSourceTexture = () => {
            let (
              ...
              BasicSourceTextureRenderWorkerTool.prepareStateAndCreateTwoMaps(sandbox);
            let state =
          ...
          let _prepareForArrayBufferViewSourceTexture = () => {
            let (state, (map1, map2), (source1, source2)) =
              ArrayBufferViewSourceTextureRenderWorkerTool.prepareStateAndCreateTwoMaps(sandbox);
            let state =
              state |> FakeGlWorkerTool.setFakeGl(FakeGlWorkerTool.buildFakeGl(~sandbox, ()));
                ...
                () => {
                  let _initTwoGameObjects = (map1, map2, state) => {
                    ...
                    let (state, gameObject2, material2) =
                      BasicMaterialTool.createGameObject(state);
                    let state = state |> BasicMaterialAPI.setBasicMaterialMap(material1, map1);
                    let state = state |> BasicMaterialAPI.setBasicMaterialMap(material2, map2);
                        ...
                        () => {
                          ...
                          |> MainInitJobMainWorkerTool.test(
                               ...
                               (postMessageToRenderWorker) => {
                                 ...
                                 needInitedTextureIndexArray |> Js.Array.length |> expect == 0
                               }
                             )
                        }
                      )
