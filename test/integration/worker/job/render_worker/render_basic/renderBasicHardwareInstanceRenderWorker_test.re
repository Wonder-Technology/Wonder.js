...
let _ =
  describe("test render basic hardware instance in render worker", () => {
    open Expect;
    ...
    describe("use program", () => {
      ...
      testPromise("create program and use program only once", () => {
        let (state, program, createProgram, useProgram) =
          _prepareForUseProgram(sandbox, state^);
        RenderJobsRenderWorkerTool.initWithJobAndMainLoopAndRender(
          ~state,
          ~sandbox,
          ~completeFunc=
            _ => createProgram |> getCallCount |> expect == 1 |> resolve,
          ~jobFuncArr=
            RenderJobsRenderWorkerTool.getJobFuncArrExceptInitNoMaterialShader(),
          (),
        );
      });
      testPromise("only use sourceInstance's gameObject's program", () => {
        let (state, program, createProgram, useProgram) =
          _prepareForUseProgram(sandbox, state^);
        RenderJobsRenderWorkerTool.initWithJobAndMainLoopAndRender(
          ~state,
          ...
          describe("fix bug", () =>
            describe(
              ...
              describe("handle instance data position", () =>
                testPromise("vertexAttribDivisorANGLE 1", () => {
                  ...
                  RenderJobsRenderWorkerTool.initAndMainLoopAndRender(
                    ...
                    ~completeFunc=
                      _ => {
                        ...
                        let state =
                          GameObjectAPI.initGameObject(gameObject2, state);
