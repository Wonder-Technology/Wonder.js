...
let _ = describe("test render_pathTracing job", () => {
  open Expect
        ...
        groups: list{
          {
            name: "frame",
            link: Concat,
            elements: list{{name: "render_pathTracing", type_: Job}},
          },
        },
      },
      (),
    ...
    DirectorCPTool.initAndRender(
      ~handleSuccessFunc=() =>
        (
          ...
          ->SinonTool.getStub
          ->SinonTool.calledWithArg2(IWebGPUCoreDp.commandEncoderDescriptor(), device),
  ...
  testPromise("set pipeline", () => {
    ...
    let setPipelineStubData =
      createEmptyStub(refJsObjToSandbox(sandbox.contents))->SinonTool.createTwoArgsEmptyStubData
    let pass = WebGPURayTracingDependencyTool.createPassEncoderRayTracingObject()
    let beginRayTracingPassStubData =
      createEmptyStub(refJsObjToSandbox(sandbox.contents))
  ...
  testPromise("set all bind groups", () => {
    ...
    DirectorCPTool.initAndRender(~handleSuccessFunc=() => {
      ...
      (
        ...
        ->SinonTool.getStub
        ->getCall(0, _)
        ->SinonTool.calledWithArg3(bindGroupData1.setSlot, bindGroupData1.bindGroup, pass),
        setBindGroupStubData
        ->SinonTool.getStub
        ->getCall(1, _)
        ->SinonTool.calledWithArg3(bindGroupData2.setSlot, bindGroupData2.bindGroup, pass),
        setBindGroupStubData
        ->SinonTool.getStub
        ->getCall(2, _)
        ->SinonTool.calledWithArg3(bindGroupData3.setSlot, bindGroupData3.bindGroup, pass),
  ...
  testPromise("trace rays", () => {
    ...
    let traceRaysStubData =
      createEmptyStub(refJsObjToSandbox(sandbox.contents))->SinonTool.createSevenArgsEmptyStubData
    let width = 10
    let height = 20
    let getWidth = createEmptyStub(refJsObjToSandbox(sandbox.contents))->SinonTool.returns(width)
    let getHeight = createEmptyStub(refJsObjToSandbox(sandbox.contents))->SinonTool.returns(height)
    let pass = WebGPURayTracingDependencyTool.createPassEncoderRayTracingObject()
    let beginRayTracingPassStubData =
      createEmptyStub(refJsObjToSandbox(sandbox.contents))
      ->SinonTool.returns(pass)
    ...
    WebGPUDependencyTool.build(~sandbox, ~getWidth, ~getHeight, ())->WebGPUDependencyTool.set
    WebGPURayTracingDependencyTool.build(
      ~sandbox,
