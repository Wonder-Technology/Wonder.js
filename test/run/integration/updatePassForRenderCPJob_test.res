...
let _ = describe("test update_pass_for_render job", () => {
  open Expect
        ...
        groups: list{
          {
            name: "frame",
            link: Concat,
            elements: list{{name: "update_pass_for_render", type_: Job}},
          },
        },
      },
      (),
  ...
  describe("update common buffer data", () => {
    ...
    testPromise("set total sample count to buffer data", () => {
      ...
      DirectorCPTool.initAndRender(~handleSuccessFunc=() => {
        ...
        typeArr->Js.Typed_array.Uint32Array.unsafe_get(1)->expect == totalSampleCount
      }, ())
    })
    testPromise("set buffer's data", () => {
      ...
      DirectorCPTool.initAndRender(~handleSuccessFunc=() => {
        ...
        ->SinonTool.getStub
        ->expect
        ->SinonTool.toCalledWith((0, typeArr, buffer->Wonderjs.UniformBufferVO.value))
      }, ())
    })
  })
})
