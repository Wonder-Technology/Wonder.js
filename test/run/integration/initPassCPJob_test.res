...
let _ = describe("test init_pass job", () => {
  open Expect
        ...
        groups: list{
          {
            name: "frame",
            link: Concat,
            elements: list{{name: "init_pass", type_: Job}},
          },
        },
      },
      (),
  ...
  describe("build pixel buffer data and set to po", () => testPromise("create pixel buffer", () => {
      ...
      DirectorCPTool.init(~handleSuccessFunc=() => {
        ...
        (
          ...
          ->SinonTool.getStub
          ->getCall(0, _)
          ->SinonTool.calledWithArg2({"size": bufferSize, "usage": storage}, device),
  ...
  describe("build resolution buffer data and set to po", () => {
    let _prepare = sandbox => {
      ...
      (
        ...
        (getWidth, getHeight, createBufferStubData, setSubFloat32DataStubData),
      )
    }
    ...
    testPromise("create type arr and set it's data", () => {
      ...
      DirectorCPTool.init(~handleSuccessFunc=() => {
        ...
        typeArr->expect ==
          Js.Typed_array.Float32Array.make([width->Belt.Float.fromInt, height->Belt.Float.fromInt])
      }, ())
    })
