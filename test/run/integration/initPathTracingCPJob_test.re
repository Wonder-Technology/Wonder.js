...
let _ =
  describe("test init_pathTracing job", () => {
    ...
    let _prepare = sandbox => {
      let (repo, directionLightData) =
        DirectionLightTool.buildDirectionLightRepoWithOneDirectionLight(
          sandbox,
        );
        ...
        describe("build direction light buffer data", () => {
          describe("contract check", () => {
            testPromise("should only has one direction light", () => {
              ...
              SceneGraphRepoDependencyTool.build(
                ~sandbox,
                ~directionLightRepo=
                  DirectionLightTool.buildDirectionLightRepoWithTwoDirectionLights(
                    sandbox,
                  )
          ...
          describe("set buffer's data", () => {
            testPromise("test buffer's data", () => {
              let ((device, _), (light, (color, intensity, direction))) =
                _prepare(sandbox);
