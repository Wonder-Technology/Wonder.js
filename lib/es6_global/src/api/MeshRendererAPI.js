...
function createMeshRenderer(state) {
  var match = CreateMeshRendererService$Wonderjs.create(state[/* meshRendererRecord */24]);
...
function unsafeGetMeshRendererGameObject(meshRenderer, state) {
  Contract$WonderLog.requireCheck((function () {
          return AliveComponentService$Wonderjs.checkComponentShouldAlive(meshRenderer, DisposeMeshRendererService$Wonderjs.isAlive, state[/* meshRendererRecord */24]);
        }), IsDebugMainService$Wonderjs.getIsDebug(StateDataMain$Wonderjs.stateData));
  return GameObjectMeshRendererService$Wonderjs.unsafeGetGameObject(meshRenderer, state[/* meshRendererRecord */24]);
}
