...
function isSupportRenderWorkerAndSharedArrayBuffer() {
  var isSupportSharedArrayBuffer = Worker$Wonderjs.isSupportSharedArrayBuffer(/* () */0);
  var match = !isSupportSharedArrayBuffer;
  if (match) {
    return false;
  } else {
    return Curry._2(DetectService$Wonderjs.hasProperty, "transferControlToOffscreen", DomService$Wonderjs.buildCanvas(/* () */0));
  }
}
