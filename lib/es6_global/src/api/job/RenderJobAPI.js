...
function sendUniformRenderObjectModelData(gl, shaderIndex, transformIndex, state) {
  var renderState = CreateRenderStateMainService$Wonderjs.createRenderState(state);
  RenderJobUtils$Wonderjs.sendUniformRenderObjectModelData(gl, shaderIndex, transformIndex, CreateGetRenederDataSubStateRenderService$Wonderjs.createState(renderState), renderState);
  return state;
}

function sendUniformRenderObjectMaterialData(gl, shaderIndex, materialIndex, state) {
  var renderState = CreateRenderStateMainService$Wonderjs.createRenderState(state);
  RenderJobUtils$Wonderjs.sendUniformRenderObjectMaterialData(gl, shaderIndex, materialIndex, CreateGetRenederDataSubStateRenderService$Wonderjs.createState(renderState), renderState);
  return state;
}
