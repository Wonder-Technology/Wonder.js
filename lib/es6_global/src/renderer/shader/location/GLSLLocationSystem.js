...
function _getGLSLLocationData(state) {
  return state[/* glslLocationData */24];
}
...
function getAttributeLocationMap(shaderIndex, state) {
  return SparseMapSystem$WonderCommonlib.get(shaderIndex, state[/* glslLocationData */24][/* attributeLocationMap */0]);
}

function setAttributeLocationMap(shaderIndex, attributeLocationMap, state) {
  SparseMapSystem$WonderCommonlib.set(shaderIndex, attributeLocationMap, state[/* glslLocationData */24][/* attributeLocationMap */0]);
  return state;
}

function getUniformLocationMap(shaderIndex, state) {
  return SparseMapSystem$WonderCommonlib.get(shaderIndex, state[/* glslLocationData */24][/* uniformLocationMap */1]);
}

function setUniformLocationMap(shaderIndex, uniformLocationMap, state) {
  SparseMapSystem$WonderCommonlib.set(shaderIndex, uniformLocationMap, state[/* glslLocationData */24][/* uniformLocationMap */1]);
  return state;
}
...
function restore(intersectShaderIndexDataArray, currentState, targetState) {
  var match = currentState[/* glslLocationData */24];
  var newrecord = targetState.slice();
  newrecord[/* glslLocationData */24] = /* record */[
