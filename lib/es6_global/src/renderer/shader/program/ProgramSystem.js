...
function getProgram(shaderIndex, state) {
  return SparseMapSystem$WonderCommonlib.get(shaderIndex, state[/* programData */19][/* programMap */0]);
}
...
function unsafeGetProgram(shaderIndex, state) {
  return Contract$WonderLog.ensureCheck((function (program) {
              ...
              }), StateData$Wonderjs.stateData[/* isDebug */1], SparseMapSystem$WonderCommonlib.unsafeGet(shaderIndex, state[/* programData */19][/* programMap */0]));
}

function registerProgram(shaderIndex, state, program) {
  SparseMapSystem$WonderCommonlib.set(shaderIndex, program, state[/* programData */19][/* programMap */0]);
  return program;
}

function use(gl, program, state) {
  var data = state[/* programData */19];
...
function restore(intersectShaderIndexDataArray, currentState, targetState) {
  var match = currentState[/* programData */19];
  var newrecord = targetState.slice();
  newrecord[/* programData */19] = /* record */[
