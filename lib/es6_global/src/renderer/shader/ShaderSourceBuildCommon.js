...
function _buildTop(param, shaderLibDataArr) {
  var top = param[/* top */0];
  var varDeclare = param[/* varDeclare */2];
  var funcDefine = param[/* funcDefine */4];
  var body = param[/* body */5];
  return top + _generateUniformSource(shaderLibDataArr, varDeclare, funcDefine, body);
}
...
function buildGLSLSource(_, shaderLibDataArr, state) {
  ...
  vs[/* top */0] = _buildTop(vs, shaderLibDataArr);
  fs[/* top */0] = _buildTop(fs, shaderLibDataArr);
  return /* tuple */[
          _addAlllParts(vs),
          _addAlllParts(fs)
        ];
