...
function _sendShaderUniformData(gl, state) {
  return ArraySystem$Wonderjs.reduceState((function (state, shaderIndex) {
                ...
                return ArraySystem$Wonderjs.reduceState((function (state, param) {
                              ...
                              return state;
                            }), state, GLSLSenderConfigDataHandleSystem$Wonderjs.getShaderUniformSendNoCacheableData(shaderIndex, ProgramSystem$Wonderjs.use(gl, program, state)));
  ...
  if (match) {
    return ArraySystem$Wonderjs.reduceState((function (state, uid) {
                  if (InstanceUtils$Wonderjs.isSourceInstance(uid, state)) {
                    ...
                    var state$1 = match[0];
                    GLSLSenderDrawUtils$Wonderjs.drawElement(GeometryAdmin$Wonderjs.getDrawMode(gl), GeometryAdmin$Wonderjs.getIndexType(gl), GeometryAdmin$Wonderjs.getIndexTypeSize(gl), GeometryAdmin$Wonderjs.getIndicesCount(match[2], state$1), gl);
                    return state$1;
                  }
                }), state$1, match[0]);
  } else {
