...
function _buildMultiPrimitivesName(name, primitiveIndex) {
  if (name !== undefined) {
    return "" + (String(Js_primitive.valFromOption(name)) + ("_" + (String(primitiveIndex) + "")));
  }
...
function _buildNewMeshes(meshes, multiPrimitivesMeshMap) {
  return MutableSparseMapService$WonderCommonlib.reduceiValid((function (newMeshes, newMeshDataArr, _) {
