...
function isDefaultImageName(name) {
  return Js_option.isSome(Js_primitive.null_to_opt(name.match((/^image_/g))));
}

function isDefaultLightMaterialName(name) {
  return Js_option.isSome(Js_primitive.null_to_opt(name.match((/^lightMaterial_/g))));
}

function isDefaultBasicMaterialName(name) {
  return Js_option.isSome(Js_primitive.null_to_opt(name.match((/^basicMaterial_/g))));
}

function isDefaultTextureName(name) {
  return Js_option.isSome(Js_primitive.null_to_opt(name.match((/^texture_/g))));
}

function isDefaultGeometryName(name) {
  return Js_option.isSome(Js_primitive.null_to_opt(name.match((/^geometry_/g))));
}
