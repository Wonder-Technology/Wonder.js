...
function assembleWDBData(wd, binBuffer, param, state) {
  ...
  return Most.map((function (param) {
                ...
                var gameObjectArr = match[2];
                var match$1 = BuildRootGameObjectSystem$Wonderjs.build(wd, /* tuple */[
                      match[0],
                      gameObjectArr
                    ]);
                var rootGameObject = match$1[1];
                var state$1 = DisposeGameObjectNotExistInSceneSystem$Wonderjs.dispose(rootGameObject, gameObjectArr, match$1[0]);
                return /* tuple */[
                        state$1,
                        /* tuple */[
                          match[1],
