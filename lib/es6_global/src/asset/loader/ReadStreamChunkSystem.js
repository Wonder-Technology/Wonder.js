...
import * as StreamType$Wonderjs from "../data/StreamType.js";
...
function _assembleAndStartLoop(assembleData, jsonChunkLength, totalLoadedByteLength, loadedUint8ArrayArr, default11Image, handleBeforeStartLoopFunc, state) {
  if (assembleData !== undefined) {
    ...
    var match = AssembleStreamWDBSystem$Wonderjs.assemble(JSON.parse(_getJsonChunkStr(jsonChunkLength, totalLoadedByteLength, loadedUint8ArrayArr)), default11Image, state);
    var match$1 = match[3];
    var match$2 = match[2];
...
function read(param, param$1, param$2, reader) {
  ...
  return _readReader(reader).then((function (streamData) {
                  ...
                  if (match) {
                    ...
                    Log$WonderLog.logJson(/* tuple */[
                          "value",
                          value.byteLength
                        ]);
                    var loadedUint8ArrayArr$1 = ArrayService$Wonderjs.push(value, loadedUint8ArrayArr);
                ...
                Log$WonderLog.error(e);
                return Promise.reject(StreamType$Wonderjs.ReadError);
              }));
}
