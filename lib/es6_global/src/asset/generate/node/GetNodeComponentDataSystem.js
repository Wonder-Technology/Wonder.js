...
function _getMaterialData(param, materialCompoent, materialDataMap, getNameFunc, state) {
  var materialIndex = param[1];
  if (materialCompoent !== undefined) {
    var material = materialCompoent;
    var match = SparseMapService$WonderCommonlib.get(material, materialDataMap);
    if (match !== undefined) {
      var match$1 = match;
      return /* tuple */[
              match$1[0],
      ...
      return /* tuple */[
              ...
              materialData,
              materialIndex + 1 | 0,
              SparseMapService$WonderCommonlib.set(material, /* tuple */[
                    materialIndex,
                    materialData
                  ], materialDataMap)
            ];
...
function _getBasicMaterialData(param, basicMaterialDataMap, state) {
  ...
  return _getMaterialData(/* tuple */[
              gameObject,
              param[1]
            ], GetComponentGameObjectService$Wonderjs.getBasicMaterialComponent(gameObject, gameObjectRecord), basicMaterialDataMap, NameBasicMaterialMainService$Wonderjs.getName, state);
}
...
function _getLightMaterialData(param, lightMaterialDataMap, state) {
  ...
  return _getMaterialData(/* tuple */[
              gameObject,
              param[1]
            ], GetComponentGameObjectService$Wonderjs.getLightMaterialComponent(gameObject, gameObjectRecord), lightMaterialDataMap, NameLightMaterialMainService$Wonderjs.getName, state);
}
...
function _getComponentData(param, getComponentFunc, state) {
  ...
  if (match !== undefined) {
    var componentData = Js_primitive.some(Js_primitive.valFromOption(match));
    return /* tuple */[
            componentIndex,
            componentData,
            componentIndex + 1 | 0
