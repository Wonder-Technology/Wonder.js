...
function getInfo(infoIndex, infos) {
  return Contract$WonderLog.ensureCheck((function (param) {
                ...
                Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("has info data", "not"), (function () {
                        Contract$WonderLog.assertNullableExist(startIndex);
                        return Contract$WonderLog.assertNullableExist(endIndex);
                      }));
                return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("endIndex >= startIndex", "is " + (String(endIndex) + "")), (function () {
...
function setInfo(infoIndex, startIndex, endIndex, infos) {
  Contract$WonderLog.requireCheck((function () {
          Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("startIndex >= 0", "is " + (String(startIndex) + "")), (function () {
...
function getFloat32PointData(infoIndex, points, infos) {
  var match = getInfo(infoIndex, infos);
  return TypeArrayService$Wonderjs.getFloat32ArraySubarray(points, match[0], match[1]);
}
...
function getUint16PointData(infoIndex, points, infos) {
  var match = getInfo(infoIndex, infos);
  return TypeArrayService$Wonderjs.getUint16ArraySubarray(points, match[0], match[1]);
}
...
function getUint32PointData(infoIndex, points, infos) {
  var match = getInfo(infoIndex, infos);
  return TypeArrayService$Wonderjs.getUint32ArraySubarray(points, match[0], match[1]);
}
