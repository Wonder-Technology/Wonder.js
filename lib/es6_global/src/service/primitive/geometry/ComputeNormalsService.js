  ...
  var _compute = function (_index, _normals) {
    while(true) {
      ...
      if (match) {
        ...
        var va = Caml_int32.imul(indices[index], 3);
        var vb = Caml_int32.imul(indices[index + 1 | 0], 3);
        var vc = Caml_int32.imul(indices[index + 2 | 0], 3);
  ...
  var _compute = function (_index, _normals) {
    while(true) {
      ...
      if (match) {
        ...
        var va = Caml_int32.imul(indices32[index], 3);
        var vb = Caml_int32.imul(indices32[index + 1 | 0], 3);
        var vc = Caml_int32.imul(indices32[index + 2 | 0], 3);
        var pa = _getPosition(vertices, va);
        var pb = _getPosition(vertices, vb);
        var pc = _getPosition(vertices, vc);
        var v0 = Vector3Service$Wonderjs.sub(/* Float */0, pc, pb);
...
export {
  ...
  computeVertexNormalsByIndices ,
  computeVertexNormalsByIndices32 ,
