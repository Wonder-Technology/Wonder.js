// Generated by BUCKLESCRIPT VERSION 2.1.0, PLEASE EDIT WITH CARE
'use strict';

import * as GameObjectMapService$Wonderjs from "../../../../primitive/GameObjectMapService.js";

function getGameObject(geometry, param) {
  return GameObjectMapService$Wonderjs.getGameObject(geometry, param[/* gameObjectMap */4]);
}

function unsafeGetGameObject(geometry, param) {
  return GameObjectMapService$Wonderjs.unsafeGetGameObject(geometry, param[/* gameObjectMap */4]);
}

export {
  getGameObject       ,
  unsafeGetGameObject ,
  
}
/* GameObjectMapService-Wonderjs Not a pure module */
