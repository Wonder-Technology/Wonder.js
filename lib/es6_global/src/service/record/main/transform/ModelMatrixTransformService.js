...
function getLocalToWorldMatrixTypeArray(transform, localToWorldMatrices, localToWorldMatrixCacheMap) {
  var match = MutableSparseMapService$WonderCommonlib.get(transform, localToWorldMatrixCacheMap);
  if (match !== undefined) {
...
function getNormalMatrixTypeArray(transform, localToWorldMatrices, param) {
  ...
  var match = MutableSparseMapService$WonderCommonlib.get(transform, normalMatrixCacheMap);
  if (match !== undefined) {
    return Js_primitive.valFromOption(match);
  } else {
...
function setScaleByTuple(transform, parent, position, param) {
  ...
  var localToWorldMatrixCacheMap = record[/* localToWorldMatrixCacheMap */19];
  var localToWorldMatrix = getLocalToWorldMatrixTypeArray(parent, record[/* localToWorldMatrices */2], localToWorldMatrixCacheMap);
  return setLocalScaleByTuple(transform, Vector3Service$Wonderjs.transformMat4Tuple(position, Matrix4Service$Wonderjs.invert(localToWorldMatrix, GlobalTempService$Wonderjs.getFloat32Array1(param[0]))), record);
}
