// Generated by BUCKLESCRIPT VERSION 3.1.4, PLEASE EDIT WITH CARE

import * as GameObjectMapService$Wonderjs from "../../../../primitive/GameObjectMapService.js";

function getGameObject(material, param) {
  return GameObjectMapService$Wonderjs.getGameObject(material, param[/* gameObjectMap */13]);
}

function unsafeGetGameObject(material, param) {
  return GameObjectMapService$Wonderjs.unsafeGetGameObject(material, param[/* gameObjectMap */13]);
}

export {
  getGameObject ,
  unsafeGetGameObject ,
  
}
/* GameObjectMapService-Wonderjs Not a pure module */
