...
function createBuffer(gl, capacity, state) {
  var buffer = PoolVboBufferService$Wonderjs.getInstanceBuffer(gl, state[/* vboBufferRecord */0]);
...
function getOrCreateMatrixFloat32Array(sourceInstance, defaultCapacity, param, state) {
  ...
  if (match) {
    ...
    var match$1 = TypeArrayPoolService$Wonderjs.getFloat32TypeArrayFromPool(_getFloat32InstanceArraySize(capacity), state[/* typeArrayPoolRecord */1]);
    if (match$1) {
      return match$1[0];
    } else {
