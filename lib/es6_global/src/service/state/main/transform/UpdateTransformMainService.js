...
function updateAndSetRotationByTuple(transform, rotation, globalTempRecord, record) {
  var match = HierachyTransformService$Wonderjs.getParent(transform, record);
  if (match) {
    return ModelMatrixTransformService$Wonderjs.setLocalRotationByTuple(transform, QuaternionService$Wonderjs.multiply(QuaternionService$Wonderjs.invert(updateAndGetRotationTuple(match[0], globalTempRecord, record)), rotation), record);
  } else {
