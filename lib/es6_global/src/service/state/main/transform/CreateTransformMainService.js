...
function _initDataWhenCreate(index, transformRecord) {
  ...
  if (match !== 0) {
    ...
    newrecord[/* childMap */10] = _setDefaultChildren(index, childMap);
    return newrecord;
  }
}
...
function createWithoutMarkNotDirtyWithRecord(settingRecord, transformRecord) {
  ...
  transformRecord$1[/* disposedIndexArray */15] = match[2];
  return BufferService$Wonderjs.checkNotExceedMaxCount(BufferSettingService$Wonderjs.getTransformDataBufferCount(settingRecord), /* tuple */[
              transformRecord$1,
              index$1
            ]);
}
...
function createWithoutMarkNotDirty(state) {
  ...
  state[/* transformRecord */11] = /* Some */[match[0]];
  return /* tuple */[
          state,
          match[1]
        ];
...
function create(state) {
  ...
  var index = match[1];
  var state$1 = match[0];
  state$1[/* transformRecord */11] = /* Some */[DirtyTransformService$Wonderjs.mark(index, /* true */1, RecordTransformMainService$Wonderjs.getRecord(state$1))];
  return /* tuple */[
          state$1,
          index
        ];
