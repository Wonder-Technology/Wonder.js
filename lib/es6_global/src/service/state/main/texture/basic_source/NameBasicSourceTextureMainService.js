...
function getName(texture, state) {
  return NameService$Wonderjs.getName(texture, RecordBasicSourceTextureMainService$Wonderjs.getRecord(state)[/* nameMap */16]);
}

function unsafeGetName(texture, state) {
  return NameService$Wonderjs.unsafeGetName(texture, RecordBasicSourceTextureMainService$Wonderjs.getRecord(state)[/* nameMap */16]);
}
...
function setName(texture, name, state) {
  ...
  newrecord[/* basicSourceTextureRecord */18] = /* record */[
    ...
    /* nameMap */NameService$Wonderjs.setName(texture, name, record[/* nameMap */16]),
    /* materialsMap */record[/* materialsMap */17]
  ];
  return newrecord;
}
