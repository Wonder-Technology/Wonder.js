...
function isAlive(geometry, param) {
  var disposedIndexArray = param[/* disposedIndexArray */14];
...
function _disposeData(geometry, customGeometryRecord) {
  var disposeCount = customGeometryRecord[/* disposeCount */11];
  var disposedIndexArray = customGeometryRecord[/* disposedIndexArray */14];
  var disposedIndexMap = customGeometryRecord[/* disposedIndexMap */15];
  var newrecord = customGeometryRecord.slice();
  newrecord[/* disposeCount */11] = disposeCount + 1 | 0;
  newrecord[/* gameObjectMap */12] = DisposeComponentService$Wonderjs.disposeSparseMapData(geometry, customGeometryRecord[/* gameObjectMap */12]);
  newrecord[/* groupCountMap */13] = DisposeComponentService$Wonderjs.disposeSparseMapData(geometry, customGeometryRecord[/* groupCountMap */13]);
  newrecord[/* disposedIndexArray */14] = ArrayService$Wonderjs.push(geometry, disposedIndexArray);
  newrecord[/* disposedIndexMap */15] = SparseMapService$WonderCommonlib.set(geometry, /* true */1, disposedIndexMap);
  return newrecord;
}
...
function handleBatchDisposeComponent(geometryArray, state) {
  ...
  return /* tuple */[
          (newrecord[/* customGeometryRecord */21] = /* Some */[match[1]], newrecord),
          match[0]
        ];
}
