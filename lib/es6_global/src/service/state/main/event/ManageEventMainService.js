...
function onCustomGameObjectEvent(eventName, handleFunc, target, state, $staropt$star, _) {
  var priority = $staropt$star !== undefined ? $staropt$star : 0;
...
function setDomEventStreamSubscription(domEventStreamSubscription, state) {
  ...
  newrecord[/* eventRecord */40] = /* record */[
    /* domEventStreamSubscription */Js_primitive.some(domEventStreamSubscription),
...
function unsubscribeDomEventStream(state) {
  var match = state[/* eventRecord */40][/* domEventStreamSubscription */0];
  if (match !== undefined) {
    _unsubscribeDomEventStream(Js_primitive.valFromOption(match));
    return state;
  } else {
    return state;
  }
