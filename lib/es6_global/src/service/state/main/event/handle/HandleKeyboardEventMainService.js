...
function _handleShiftKey(keyCode, $$char, param) {
  var match = SparseMapService$WonderCommonlib.get(keyCode, param[0]);
  if (match !== undefined) {
    return match;
  } else {
    var match$1 = HashMapService$WonderCommonlib.get($$char, param[1]);
...
function execEventHandle(eventName, keyboardDomEvent, state) {
  var match = SparseMapService$WonderCommonlib.get(eventName, state[/* eventRecord */41][/* keyboardDomEventDataArrMap */2]);
  if (match !== undefined) {
