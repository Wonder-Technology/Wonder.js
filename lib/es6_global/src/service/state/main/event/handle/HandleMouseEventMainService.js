...
function getMovementDelta(mouseDomEvent, state) {
  ...
  if (match) {
    ...
    return HandlePointDomEventMainService$Wonderjs.getMovementDelta(getLocation(mouseDomEvent, state), MouseEventService$Wonderjs.getLastXY(state[/* eventRecord */40]), state);
  }
}
...
function execEventHandle(mouseEvent, state) {
  var match = MutableSparseMapService$WonderCommonlib.get(mouseEvent[/* name */0], state[/* eventRecord */40][/* mouseDomEventDataArrMap */1]);
  if (match !== undefined) {
