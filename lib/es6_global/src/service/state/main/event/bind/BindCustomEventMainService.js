...
function _addToEventArr(eventName, eventData, eventArrMap) {
  ...
  if (match) {
    return HashMapService$WonderCommonlib.set(eventName, _addEventDataByPriority(eventData, match[0]), eventArrMap);
  } else {
...
function _removeFromEventArrMapByHandleFunc(eventName, handleFunc, eventArrMap) {
  var match = HashMapService$WonderCommonlib.get(eventName, eventArrMap);
  if (match) {
    return HashMapService$WonderCommonlib.set(eventName, _removeFromEventArrByHandleFunc(match[0], handleFunc), eventArrMap);
  } else {
    return eventArrMap;
  }
}
...
function bindGameObjectEvent(param, handleFunc, state) {
  ...
  var tmp;
  if (match) {
    var targetEventArrMap = match[0];
    var match$1 = SparseMapService$WonderCommonlib.get(target, targetEventArrMap);
    tmp = match$1 ? HashMapService$WonderCommonlib.set(eventName, SparseMapService$WonderCommonlib.set(target, _addEventDataByPriority(eventData, match$1[0]), targetEventArrMap), customGameObjectEventArrMap) : HashMapService$WonderCommonlib.set(eventName, SparseMapService$WonderCommonlib.set(target, /* array */[eventData], targetEventArrMap), customGameObjectEventArrMap);
  } else {
...
function unbindGameObjectEventByTarget(param, state) {
  ...
  newrecord[/* eventRecord */40] = /* record */[
    ...
    /* touchDomEventDataArrMap */eventRecord[/* touchDomEventDataArrMap */3],
    /* customGlobalEventArrMap */eventRecord[/* customGlobalEventArrMap */4],
    /* customGameObjectEventArrMap */match ? SparseMapService$WonderCommonlib.deleteVal(param[1], match[0]) : customGameObjectEventArrMap,
    /* mouseEventData */eventRecord[/* mouseEventData */6],
    /* keyboardEventData */eventRecord[/* keyboardEventData */7],
    /* touchEventData */eventRecord[/* touchEventData */8]
  ];
...
function unbindGameObjectEventByHandleFunc(param, handleFunc, state) {
  ...
  if (match) {
    var targetEventArrMap = match[0];
    var match$1 = SparseMapService$WonderCommonlib.get(target, targetEventArrMap);
    tmp = match$1 ? HashMapService$WonderCommonlib.set(eventName, SparseMapService$WonderCommonlib.set(target, _removeFromEventArrByHandleFunc(match$1[0], handleFunc), targetEventArrMap), customGameObjectEventArrMap) : customGameObjectEventArrMap;
  } else {
