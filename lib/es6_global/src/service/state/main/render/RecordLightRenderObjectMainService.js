...
function _initBufferData(count) {
  var buffer = new SharedArrayBuffer(Caml_int32.imul(count, Caml_int32.imul(Uint32Array.BYTES_PER_ELEMENT, Caml_int32.imul(RenderObjectBufferTypeArrayService$Wonderjs.getComponentSize(/* () */0), 5)) + Caml_int32.imul(Uint8Array.BYTES_PER_ELEMENT, RenderObjectBufferTypeArrayService$Wonderjs.getGeometryTypeSize(/* () */0)) | 0));
  return /* tuple */[
          buffer,
          CreateTypeArrayRenderObjectService$Wonderjs.setDefaultTypeArrData(count, CreateTypeArrayRenderObjectService$Wonderjs.createTypeArrays(buffer, count))
        ];
}

function create(state) {
  var lightMaterialDataBufferCount = BufferSettingService$Wonderjs.getLightMaterialDataBufferCount(state[/* settingRecord */0]);
  var match = _initBufferData(lightMaterialDataBufferCount);
  var match$1 = match[1];
  return /* Some */[/* record */[
            /* buffer */match[0],
