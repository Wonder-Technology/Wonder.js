...
function detect(state) {
  ...
  newrecord[/* workerDetectRecord */39] = /* record */[
    ...
    /* isSupportRenderWorkerAndSharedArrayBuffer */match ? false : Curry._2(DetectService$Wonderjs.hasProperty, "transferControlToOffscreen", DomService$Wonderjs.buildCanvas(/* () */0))
  ];
  return newrecord;
}

function isSupportSharedArrayBuffer(state) {
  return state[/* workerDetectRecord */39][/* isSupportSharedArrayBuffer */0];
}
