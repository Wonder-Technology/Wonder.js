...
function reInitComponents(materialIndices, param, param$1, state) {
  ...
  return ArrayService$WonderCommonlib.reduceOneParam((function (state, materialIndex) {
                ...
                var match = ShaderIndexAllShaderService$Wonderjs.isDefaultShaderIndex(currentShaderIndex);
                if (!match) {
                  ShaderIndexAllShaderService$Wonderjs.removeShaderIndexFromMaterial(currentShaderIndex, materialIndex, state[/* shaderRecord */28], state[/* glslSenderRecord */32]);
                }
                var match$1 = WorkerDetectMainService$Wonderjs.isUseWorker(state);
                if (match$1) {
