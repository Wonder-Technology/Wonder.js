...
function _sendTransformMatrixDataBufferData(glDataTuple, shaderIndex, stride, sendRenderDataSubState, state) {
...
function _updateAndSendTransformMatrixDataBufferData(glDataTuple, shaderIndex, param, sendRenderDataSubState, state) {
  InstanceBufferRenderService$Wonderjs.updateData(glDataTuple[0], param[1], param[2]);
  return _sendTransformMatrixDataBufferData(glDataTuple, shaderIndex, param[0], sendRenderDataSubState, state);
}
...
function _sendTransformMatrixData(param, param$1, fillMatrixTypeArrFunc, state) {
  ...
  return _updateAndSendTransformMatrixDataBufferData(/* tuple */[
              ...
              match$3[0]
            ], CreateSendRenederDataSubStateRenderService$Wonderjs.createState(state), state);
}
...
function _sendStaticTransformMatrixData(componentTuple, dataTuple, fillMatrixTypeArrFunc, state) {
  ...
  if (match$3) {
    ...
    return _sendTransformMatrixDataBufferData(/* tuple */[
                gl,
                match$2[1]
              ], match$2[2], match$1[2], sendRenderDataSubState, state);
  } else {
