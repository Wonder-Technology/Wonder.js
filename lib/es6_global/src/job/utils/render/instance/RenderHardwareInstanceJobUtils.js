...
function _sendTransformMatrixDataBufferData(glDataTuple, shaderIndex, stride, state) {
  return ArrayService$WonderCommonlib.reduceOneParami((function (state, sendData, index) {
              ...
              }), state, HandleAttributeConfigDataService$Wonderjs.unsafeGetInstanceAttributeSendData(shaderIndex, state[/* glslSenderRecord */2]));
}
...
function _sendStaticTransformMatrixData(componentTuple, dataTuple, fillMatrixTypeArrFunc, state) {
  ...
  var match$3 = MarkIsSendTransformMatrixDataService$Wonderjs.isSendTransformMatrixData(sourceInstance, state[/* sourceInstanceRecord */13][/* isSendTransformMatrixDataMap */5]);
  if (match$3 !== 0) {
    ...
    newrecord[/* sourceInstanceRecord */13] = StaticRenderSourceInstanceService$Wonderjs.markIsSendTransformMatrixData(sourceInstance, /* true */1, state$1[/* sourceInstanceRecord */13]);
    return newrecord;
  }
}
...
function _sendDynamicTransformMatrixData(componentTuple, dataTuple, fillMatrixTypeArrFunc, state) {
  ...
  return _sendTransformMatrixData(componentTuple, dataTuple, fillMatrixTypeArrFunc, (newrecord[/* sourceInstanceRecord */13] = StaticRenderSourceInstanceService$Wonderjs.markIsSendTransformMatrixData(componentTuple[1], /* false */0, state[/* sourceInstanceRecord */13]), newrecord));
}

function _geMatrixMapTuple(state) {
  var match = state[/* vboBufferRecord */0];
  var match$1 = state[/* sourceInstanceRecord */13];
  return /* tuple */[
...
function _prepareData(gl, shaderIndex, param, state) {
  var extension = GPUDetectService$Wonderjs.unsafeGetInstanceExtension(state[/* gpuDetectRecord */14]);
...
function render(gl, param, param$1, state) {
  ...
  var gl$1 = match[0];
  var match$1 = StaticRenderSourceInstanceService$Wonderjs.isTransformStatic(sourceInstance, state$1[/* sourceInstanceRecord */13]);
  var state$2 = match$1 !== 0 ? _sendStaticTransformMatrixData(/* tuple */[
          transformIndex,
          sourceInstance
