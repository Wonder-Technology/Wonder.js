...
function execJob(_, e, stateData) {
  return MostUtils$Wonderjs.callFunc((function () {
                ...
                var match = OptionService$Wonderjs.isJsonSerializedValueNone(imguiData.imguiFunc);
                if (match) {
                  return e;
                } else {
                  var imguiRecord = RecordIMGUIService$WonderImgui.setControlData(imguiData.controlData, ManageIMGUIAPI$WonderImgui.setIMGUIFunc(OptionService$Wonderjs.unsafeGetJsonSerializedValue(imguiData.customData), SerializeService$Wonderjs.deserializeFunction(OptionService$Wonderjs.unsafeGetJsonSerializedValue(imguiData.imguiFunc)), RecordIMGUIRenderWorkerService$Wonderjs.getRecord(state)));
                  state[/* imguiRecord */27] = imguiRecord;
                  var state$1 = ManageIMGUIService$WonderImgui.render(DeviceManagerService$Wonderjs.unsafeGetGl(state[/* deviceManagerRecord */4]), imguiData.ioData, RecordAPIRenderWorkerService$Wonderjs.getAPIJsObj(state), /* tuple */[
