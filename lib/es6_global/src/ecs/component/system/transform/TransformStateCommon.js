...
function getTransformData(state) {
  return Js_option.getExn(state[/* transformData */13]);
}

function deepCopyStateForRestore(state) {
  var match = Js_option.getExn(state[/* transformData */13]);
  var index = match[/* index */0];
  var disposedIndexArray = match[/* disposedIndexArray */7];
  var newrecord = state.slice();
  newrecord[/* transformData */13] = /* Some */[/* record */[
      /* index */index,
...
function restore(currentState, sharedData, targetState) {
  var match = Js_option.getExn(currentState[/* transformData */13]);
